...
from src.platform import FrameCapture, Rect, WindowSession, WindowInfo
...
class FakeOCRReader:
    def read_lines(self, frame: FrameCapture, region_name: str, rect: Rect | None = None) -> tuple[OCRResult, ...]:
        if region_name == "battle_prompt":
            return (OCRResult(text="第1回合", confidence=0.9, region_name=region_name),)
        return ()


class DefaultObservationProviderTestCase(unittest.TestCase):
    def test_observe_collects_window_frame_matches_and_ocr(self) -> None:
        ...
        self.assertEqual(1, len(observation.ocr_texts))
