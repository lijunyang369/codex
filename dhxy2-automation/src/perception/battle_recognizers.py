...
ROUND_NUMBER_PATTERN = re.compile(r"\d+")
...
def _best_round_line(lines: tuple[OCRResult, ...]) -> tuple[OCRResult | None, str]:
    best_line: OCRResult | None = None
    best_digits = ""
    for line in lines:
        digits = "".join(ROUND_NUMBER_PATTERN.findall(line.text))
        if not digits:
            continue
        if best_line is None or line.confidence > best_line.confidence:
            best_line = line
            best_digits = digits
    return best_line, best_digits
...
class BattleRoundRecognizer:
    ...
    def recognize(self, snapshot: RecognitionSnapshot) -> RecognitionModuleResult:
        ...
        ocr_lines = snapshot.ocr_lines_for_region(self.spec.region_name)
        details["ocr_lines"] = [line.text for line in ocr_lines]
        best_line, digits = _best_round_line(ocr_lines)
        if best_line is None or not digits:
            return RecognitionModuleResult(
                module_id=self.spec.module_id,
                label=self.spec.label,
                region_name=self.spec.region_name,
        ...
        details["round_number"] = int(digits)
        details["round_text"] = best_line.text
        details["bounds"] = list(best_line.bounds) if best_line.bounds else None
        return RecognitionModuleResult(
            module_id=self.spec.module_id,
            label=self.spec.label,
            region_name=self.spec.region_name,
