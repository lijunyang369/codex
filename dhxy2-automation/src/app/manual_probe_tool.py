...
from src.app.recognition_workbench import RecognitionWorkbenchArtifacts, RecognitionWorkbenchService
from src.app.service import AppTickResult
from src.app.window_binding import resolve_window_session
from src.domain import AutomationAction, BattleObservation, BattleState, TransitionResult
from src.executor import ExecutionResult, Win32SendInputGateway
from src.perception import RecognitionModuleResult, RecognitionModuleSpec
...
class ManualCoordinateProbeApp:
    ...
    def __init__(
        ...
        self._result_var = tk.StringVar(value="ready")
        ...
        self._auto_status_var = tk.StringVar(value="idle")
        self._auto_tick_var = tk.StringVar(value="0")
        self._auto_state_var = tk.StringVar(value="NOT_STARTED")
        ...
        self._build()
        self._inject_recognition_home_button()
        self._inject_round_digit_template_button()
    ...
    def _build(self) -> None:
        ...
        ttk.Label(header, text="大话西游2 自动化工具台", font=("Microsoft YaHei UI", 18, "bold")).grid(
            row=0,
            column=0,
            sticky="w",
        )
        ...
        self._pages["home"] = self._build_home_page(body)
        self._pages["probe"] = self._build_probe_page(body)
        self._pages["recognition"] = self._build_recognition_page(body)
        self._pages["auto_battle"] = self._build_auto_battle_page(body)

        for page in self._pages.values():
            page.grid(row=0, column=0, sticky="nsew")

    def _inject_recognition_home_button(self) -> None:
        home_page = self._pages.get("home")
        if home_page is None:
            return
        children = home_page.winfo_children()
        if not children:
            return
        nav = children[0]
        ttk.Button(nav, text="识别模块测试", command=lambda: self._show_page("recognition")).pack(side=tk.LEFT, padx=(8, 0))

    def _inject_round_digit_template_button(self) -> None:
        recognition_page = self._pages.get("recognition")
        if recognition_page is None:
            return
        children = recognition_page.winfo_children()
        if not children:
            return
        toolbar = children[0]
        ttk.Button(toolbar, text="保存回合数字模板", command=self._save_round_digit_template).pack(side=tk.LEFT, padx=(8, 0))
    ...
    def _build_home_page(self, parent: ttk.Frame) -> ttk.Frame:
        ...
        nav = ttk.Frame(frame, padding=16)
        nav.grid(row=0, column=0, sticky="ew")
        ttk.Label(nav, text="入口", font=("Microsoft YaHei UI", 14, "bold")).pack(anchor="w")
        ttk.Label(nav, text="选择一个二级工具页面。").pack(anchor="w", pady=(6, 12))
        ttk.Label(nav, text="热更新已开启：配置改动自动生效，工具源码改动自动重载。").pack(anchor="w", pady=(0, 12))
        ttk.Button(nav, text="坐标探针", command=lambda: self._show_page("probe")).pack(side=tk.LEFT)
        ttk.Button(nav, text="自动战斗", command=lambda: self._show_page("auto_battle")).pack(side=tk.LEFT, padx=(8, 0))

        log_frame = ttk.LabelFrame(frame, text="入口日志", padding=10)
        log_frame.grid(row=1, column=0, sticky="nsew", pady=(12, 0))
        log_frame.columnconfigure(0, weight=1)
        log_frame.rowconfigure(0, weight=1)
        widget = tk.Text(log_frame, wrap=tk.WORD, height=24, state="disabled")
        widget.grid(row=0, column=0, sticky="nsew")
        scrollbar = ttk.Scrollbar(log_frame, orient="vertical", command=widget.yview)
        scrollbar.grid(row=0, column=1, sticky="ns")
        widget.configure(yscrollcommand=scrollbar.set)
        self._home_log_widget = widget
        return frame

    def _build_probe_page(self, parent: ttk.Frame) -> ttk.Frame:
        frame = ttk.Frame(parent)
        ...
        ttk.Button(toolbar, text="返回主页", command=lambda: self._show_page("home")).pack(side=tk.LEFT)
        ttk.Button(toolbar, text="刷新配置", command=self._reload_entries).pack(side=tk.LEFT, padx=(8, 0))
        ...
        return frame
    def _build_recognition_page(self, parent: ttk.Frame) -> ttk.Frame:
        frame = ttk.Frame(parent)
        frame.columnconfigure(1, weight=1)
        frame.columnconfigure(2, weight=1)
        frame.rowconfigure(1, weight=1)
        ...
        toolbar.grid(row=0, column=0, columnspan=3, sticky="ew", pady=(0, 12))
        ttk.Button(toolbar, text="返回主页", command=lambda: self._show_page("home")).pack(side=tk.LEFT)
        ttk.Button(toolbar, text="检测当前模块", command=self._run_selected_recognition_capture).pack(side=tk.LEFT, padx=(8, 0))
        ttk.Button(toolbar, text="检测全部模块", command=self._run_recognition_capture).pack(side=tk.LEFT, padx=(8, 0))
        ttk.Button(toolbar, text="保存截图区域", command=self._save_recognition_region).pack(side=tk.LEFT, padx=(8, 0))
        ttk.Button(toolbar, text="保存回合数字模板", command=self._save_round_digit_template).pack(side=tk.LEFT, padx=(8, 0))

        left_panel = ttk.LabelFrame(frame, text="识别模块", padding=8)
        left_panel.grid(row=1, column=0, sticky="nsw", padx=(0, 16))
        listbox = tk.Listbox(left_panel, width=34, height=20)
        listbox.pack(fill=tk.BOTH, expand=True)
        listbox.bind("<<ListboxSelect>>", self._on_select_recognition_module)
        self._recognition_listbox = listbox
        ...
        self._add_readonly_entry(right_panel, 0, "模块 ID", self._recognition_module_id_var)
        self._add_readonly_entry(right_panel, 1, "区域", self._recognition_region_var)
        self._add_readonly_entry(right_panel, 2, "模式", self._recognition_mode_var)
        self._add_readonly_entry(right_panel, 3, "识别结果", self._recognition_detected_var)
        self._add_readonly_entry(right_panel, 4, "置信度", self._recognition_confidence_var)
        self._add_readonly_entry(right_panel, 5, "摘要", self._recognition_summary_text_var)
        self._add_readonly_entry(right_panel, 6, "Scenario", self._recognition_config_path_var)
        self._add_entry(right_panel, 7, "Left", self._recognition_left_var)
        self._add_entry(right_panel, 8, "Top", self._recognition_top_var)
        self._add_entry(right_panel, 9, "Right", self._recognition_right_var)
        self._add_entry(right_panel, 10, "Bottom", self._recognition_bottom_var)
        self._add_entry(right_panel, 11, "Round Digit", self._recognition_round_digit_var)
        self._add_readonly_entry(right_panel, 13, "Full Image", self._recognition_full_path_var)
        self._add_readonly_entry(right_panel, 14, "Region Image", self._recognition_crop_path_var)
        self._add_readonly_entry(right_panel, 15, "Template Image", self._recognition_template_path_var)
        self._add_readonly_entry(right_panel, 16, "Template Status", self._recognition_round_template_status_var)
        self._add_readonly_entry(right_panel, 17, "Record", self._recognition_record_path_var)

        log_frame = ttk.LabelFrame(right_panel, text="模块详情", padding=8)
        log_frame.grid(row=18, column=0, columnspan=2, sticky="nsew", pady=(12, 0))
        log_frame.columnconfigure(0, weight=1)
        log_frame.rowconfigure(0, weight=1)
        widget = tk.Text(log_frame, wrap=tk.WORD, height=12, state="disabled")
        widget.grid(row=0, column=0, sticky="nsew")
        scrollbar = ttk.Scrollbar(log_frame, orient="vertical", command=widget.yview)
        scrollbar.grid(row=0, column=1, sticky="ns")
        widget.configure(yscrollcommand=scrollbar.set)
        self._recognition_log_widget = widget

        preview_panel = ttk.LabelFrame(frame, text="预览", padding=8)
        ...
        ttk.Label(preview_panel, text="Full").grid(row=0, column=0, sticky="w")
        self._recognition_full_preview_label = ttk.Label(preview_panel, text="暂无图片", anchor="center")
        self._recognition_full_preview_label.grid(row=1, column=0, sticky="nsew", pady=(0, 8))
        ttk.Label(preview_panel, text="Region").grid(row=2, column=0, sticky="w")
        self._recognition_crop_preview_label = ttk.Label(preview_panel, text="暂无图片", anchor="center")
        self._recognition_crop_preview_label.grid(row=3, column=0, sticky="nsew", pady=(0, 8))
        ttk.Label(preview_panel, text="Template").grid(row=4, column=0, sticky="w")
        self._recognition_template_preview_label = ttk.Label(preview_panel, text="暂无模板", anchor="center")
        self._recognition_template_preview_label.grid(row=5, column=0, sticky="nsew")
        return frame
        return frame

    def _build_auto_battle_page(self, parent: ttk.Frame) -> ttk.Frame:
        frame = ttk.Frame(parent)
        frame.columnconfigure(0, weight=1)
        frame.rowconfigure(2, weight=1)
        ...
        ttk.Button(toolbar, text="返回主页", command=lambda: self._show_page("home")).pack(side=tk.LEFT)
        ttk.Checkbutton(toolbar, text="开启自动战斗", variable=self._auto_enabled_var).pack(side=tk.LEFT, padx=(12, 0))
        ttk.Button(toolbar, text="启动", command=self._start_auto_battle).pack(side=tk.LEFT, padx=(8, 0))
        ttk.Button(toolbar, text="停止", command=self._stop_auto_battle).pack(side=tk.LEFT, padx=(8, 0))
        ttk.Button(toolbar, text="清空日志", command=self._clear_battle_log).pack(side=tk.LEFT, padx=(8, 0))

        status = ttk.LabelFrame(frame, text="运行状态", padding=10)
        status.grid(row=1, column=0, sticky="ew")
        status.columnconfigure(1, weight=1)
        self._add_readonly_entry(status, 0, "状态", self._auto_status_var)
        self._add_readonly_entry(status, 1, "Tick", self._auto_tick_var)
        self._add_readonly_entry(status, 2, "Battle State", self._auto_state_var)
        self._add_readonly_entry(status, 3, "Feedback", self._auto_feedback_var)
        self._add_readonly_entry(status, 4, "Session", self._auto_session_var)
        self._add_readonly_entry(status, 5, "Runtime Log", self._auto_runtime_log_var)

        log_frame = ttk.LabelFrame(frame, text="战斗日志", padding=10)
        log_frame.grid(row=2, column=0, sticky="nsew", pady=(12, 0))
        log_frame.columnconfigure(0, weight=1)
        log_frame.rowconfigure(0, weight=1)
        widget = tk.Text(log_frame, wrap=tk.WORD, height=28, state="disabled")
        widget.grid(row=0, column=0, sticky="nsew")
        scrollbar = ttk.Scrollbar(log_frame, orient="vertical", command=widget.yview)
        scrollbar.grid(row=0, column=1, sticky="ns")
        widget.configure(yscrollcommand=scrollbar.set)
        self._battle_log_widget = widget
        return frame
    ...
    def _save_feedback(self) -> None:
        ...
        self._result_var.set(f"saved feedback: {record_path.name}")
    ...
    def _start_auto_battle(self) -> None:
        if not self._auto_enabled_var.get():
            self._auto_status_var.set("disabled")
            self._append_battle_log("auto_battle is disabled by checkbox")
        ...
        try:
            ...
            self._auto_status_var.set("start_failed")
            self._append_battle_log(f"start_failed error={exc}")
            self._append_home_log("ERROR", f"auto_battle_start_failed error={exc}")
            return

        self._auto_status_var.set("running")
        self._auto_session_var.set(session_root)
        self._auto_runtime_log_var.set(runtime_log)
        self._last_battle_scene = ""
        self._last_battle_round_logged = 0
        self._clear_battle_log()
        self._append_battle_log(f"auto_battle_started session={session_root}")
    ...
    def _stop_auto_battle(self) -> None:
        ...
        self._auto_status_var.set("stopped")
        self._last_battle_scene = ""
        self._last_battle_round_logged = 0
        self._append_battle_log("auto_battle_stopped")
    ...
    def _run_auto_battle_tick(self) -> None:
        ...
        try:
            ...
            self._auto_status_var.set("tick_failed")
            self._append_battle_log(f"tick_failed error={exc}")
            self._append_home_log("ERROR", f"auto_battle_tick_failed error={exc}")
            self._auto_battle_service.stop()
            return

        self._auto_tick_var.set(str(report.tick_index))
        self._auto_state_var.set(report.state)
        self._auto_feedback_var.set(report.feedback_reason or "none")
        ...
        if report.finished:
            self._auto_status_var.set("finished")
    ...
    def _append_battle_report(self, report: AutoBattleTickReport) -> None:
        ...
        round_number = report.observation.round_number or (report.round_index + 1)
        if round_number != self._last_battle_round_logged:
            self._append_battle_log(f"识别到新回合：第 {round_number} 回合")
            self._last_battle_round_logged = round_number
