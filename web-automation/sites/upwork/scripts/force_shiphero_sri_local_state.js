...
const expression = `(() => {
  ...
  return {
    ...
    dropdownStates: dropdowns.map((dd) => ({
      selected: dd.internalSelected,
      title: dd.$props.title,
    })),
    labels: [...document.querySelectorAll('.air3-dropdown-toggle-label')].map((el) => (el.innerText || '').trim()).slice(0, 6),
  };
})()`;
